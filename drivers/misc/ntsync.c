 ...
 *
 * Backported to Linux 4.19 by removing dependency on:
 *   - kzalloc_obj / kmalloc_flex (6.x allocator helpers)
 *   - FD_PREPARE / fd_prepare_file / fd_publish (6.x fd helpers)
 *   - lockdep_assert(cond) bare form (added in 5.x)
 *   - LOCK_STATE_NOT_HELD enum constant (added in 5.x)
 */
...
/*
 * Backport shim: lockdep_assert(cond) — not present in 4.19 lockdep.h.
 * LOCK_STATE_NOT_HELD is also a 5.x+ enum; in 4.19 lockdep_is_held() returns
 * 0 (false) when not held, so we treat it the same way.
 */
#ifndef lockdep_assert
#ifdef CONFIG_LOCKDEP
#define lockdep_assert(cond)	do {					\
	WARN_ON(debug_locks && !(cond));				\
} while (0)
#else
...
/*
 * In 4.19 lockdep_is_held() returns 0 when not held, nonzero when held.
 * LOCK_STATE_NOT_HELD from newer kernels equals 0, so the comparison
 * "!= LOCK_STATE_NOT_HELD" is equivalent to "!= 0", i.e. nonzero / truthy.
 */
					   ...
					   enum ntsync_type type)
{
	...
	/* kzalloc_obj() is 6.x+; use kzalloc with explicit size for 4.19 */
...
/*
 * Backport note: upstream uses FD_PREPARE/fd_prepare_file/fd_publish (6.x).
 * For 4.19 we use get_unused_fd_flags() + anon_inode_getfile() + fd_install().
 */
		      ...
		      struct ntsync_q **ret_q)
{
	...
	/* kmalloc_flex() is 6.x+; use struct_size() + kmalloc() for 4.19 */
